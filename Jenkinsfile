...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="jtoh0227/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/jt-oh/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
        ...
        stage('Staging Deploy') {
            ...
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no root@3.34.123.65 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
	...
	post {
        always {
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) stg 빌드가 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        success {
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) stg 빌드가 성공적으로 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        failure {
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'danger'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) st 빌드가 실패하였습니다. Details: (<${BUILD_URL} | here >)")
    }
  }
}
